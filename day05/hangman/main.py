...
# rainbow colors shceme (from sprint 02)  
rainbow_colors = [f'\u001b[38;5;{x}m' for x in (9, 214, 11, 2, 12, 57, 13)]
def multicolor_printer(to_print, clrs):
    colorful = [clrs[i % len(clrs)] + a + RESET for i, a in enumerate(to_print)]
    print(''.join(colorful))

# score system
globalplayer1score=0
globalplayer1scorel=0

def player1score(a):
    global globalplayer1score
    globalplayer1score=globalplayer1score+a
    return globalplayer1score
def player1scorel (a):
    global globalplayer1scorel
    globalplayer1scorel=globalplayer1scorel+a
    return globalplayer1scorel
...
hagman_hello='''
        ...
        H  E  L  L  O
'''
hangman_zeroletter=CREDBG + ' ' + RESSTYLE
